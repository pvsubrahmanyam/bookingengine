...
$id = $_GET['id'];
$result = mysql_query("SELECT * FROM wp_properties WHERE id='$id'")
or die(mysql_error());
...
<form id='register'  method='post' accept-charset='UTF-8' name='form'>
    ...
    <input type='text' name='name' id='name' required=" " maxlength="50" value="<?php echo $row['Name']; ?>" />
	
    <p class="contact"><label for="name">Description:</label></p>
   <textarea style="width:400px; height:100px;" name="desc" required=" "  onBlur="checkDs(this.value)"><?php echo $row['ds'];?></textarea>
	<p class="contact"><label for="name">Phone:</label></p>
    <input type='text' name='phone' id='username' required=" " maxlength="50" value="<?php echo $row['phone']; ?>" />
	
     <p class="contact"><label for='fax' >FAX:</label></p>
     <input type='text' name='fax' id='username' required=" " maxlength="50" value="<?php echo $row['fax']; ?>" />
	 
       <p class="contact"><label for='web' >Web Address:</label></p>
    <input type='text' name='webad' id='username'  maxlength="50"onBlur="checkWb(this.value)" value="<?php echo $row['webaddr']; ?>"/>

<p class="contact"><label for='addr' >Address1:</label></p>
    <input type='text' name='addr1' id='username'  maxlength="50" required=" "onBlur="checkAd1(this.value)" value="<?php echo $row['addr1']; ?>"/>
   <td><span id="addrr1"></span></td></tr>

<p class="contact"><label for='addr' >Address2:</label></p>
    <input type='text' name='addr2' id='username' required=" " maxlength="50" onBlur="checkAd2(this.value)" value="<?php echo $row['addr2']; ?>"/>
   
    <p class="contact"><label for='sub' >Subrub:</label></p>
    <input type='text' name='subr' id='username'  maxlength="50" onBlur="checkSb(this.value)" value="<?php echo $row['suburb']; ?>"/>
	

  <p class="contact"><label for='regi'>Region:</label></p>
    <input type='text' name='region' id='username' required=" " maxlength="50" onBlur="checkRg(this.value)" value="<?php echo $row['region']; ?>"/>
   <span id="rg"></span>
    
    <p class="contact"><label for='stat' >State:</label></p>
    <input type='text' name='state' id='username' required=" " maxlength="50" onBlur="checkSt(this.value)" value="<?php echo $row['state']; ?>"/>
	<span id="st"></span>
   
     <p class="contact"><label for='pst' >Post Code:</label></p>
    <input type='text' name='postcode' id='username' required=" " maxlength="50"onBlur="checkPs(this.value)" value="<?php echo $row['postcode']; ?>"/>
	<span id="ps"></span>
 ...
 <input type='text' name='coordinates' id='username' required=" " maxlength="50" onBlur="checkCo(this.value)" value="<?php echo $row['geographicalcoordinates']; ?>"/>
	<span id="cc"></span>
    
      <p class="contact"><label for='map'>Map Zoom Level:</label></p>
    <input type='text' name='map' id='username' required=" " maxlength="30" onBlur="checkMp(this.value)" value="<?php echo $row['mapzonelevel']; ?>"/>
	<span id="mp"></span>
...
<select name="rating" id="username" maxlength="50"style="width:400px; height:27px;">
  ...
  <p class="contact"><label for='map'>Lead Time in Days:</label></p>
    <input type='text' required=" " name='ltime' id='username'  maxlength="50" onBlur="checkTm(this.value)" value="<?php echo $row['leadtime']; ?>"/>
	<span id="tt"></span>
    
   <p class="contact"><label for='map'>Availability Value:</label></p>
    <input type='text' required="" name='value' id='username'   maxlength="50" onBlur="checkVl(this.value)" value="<?php echo $row['availabletime']; ?>"/>
	<span id="vv"></span>
